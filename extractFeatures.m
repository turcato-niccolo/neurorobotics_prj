function [X] = extractFeatures(PSD, selected_freq_chan_index)
...
%
% Output arguments:
%   - X                     [windows x features] matrix containing all and only the
%   selected fetaures

num_features = length(selected_freq_chan_index);
num_windows  = size(PSD, 1);

X = nan(num_windows, num_features);
for feature_i = 1 : num_features
    ...
    X(:, feature_i) = PSD(:, freq_index, channel_index);
end
end
